...
def test_all_modules_have_pt_and_en():
    for mod in MODULES:
        pt = os.path.join(REPO_ROOT, "docs", mod, "PT", "indice.md")
        en = os.path.join(REPO_ROOT, "docs", mod, "EN", "index.md")
        assert os.path.isfile(pt), f"Missing {pt}"
        assert os.path.isfile(en), f"Missing {en}"


def test_module_03_has_estimate_example():
    path = os.path.join(REPO_ROOT, "docs", "03", "PT", "indice.md")
...
def test_module_06_api_script_present():
    path = os.path.join(REPO_ROOT, "docs", "06", "scripts", "test_api.py")
